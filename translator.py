...
def check_library(name, import_stmt):
    try:
        # `import_stmt` is a hard-coded import line from this file, not user input.
        exec(import_stmt, globals())  # nosec B102
        print(f"  ✓ {name}")
        return True
    except ImportError:
...
HAS_DOCX = check_library(
    "python-docx",
    "from docx import Document; from docx.shared import Pt, RGBColor; from docx.text.paragraph import Paragraph; from docx.oxml.shared import OxmlElement; from docx.oxml.ns import qn",
)
...
class CTranslate2Translator:
    ...
    def __init__(self, src_lang: str, tgt_lang: str):
        ...
        if src_lang == "en" and tgt_lang in self.SUPPORTED_LANGS:
            self.direction, self.tokenizer_name = (
                "en_to_x",
...
class Madlad400Translator:
    ...
    def translate_batch(self, texts: List[str]) -> List[str]:
        ...
        source_tokens = [
            self.tokenizer.convert_ids_to_tokens(self.tokenizer.encode(f"{self.tgt_prefix} {t}")) for t in texts
        ]
    ...
    def translate_batch(self, texts: List[str], batch_size: int = 16) -> List[str]:
        ...
        try:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                # Tokenize
                source_tokens = [self.tokenizer.convert_ids_to_tokens(self.tokenizer.encode(t)) for t in batch]
    ...
    def _init_providers(self, preferred: Optional[str] = None, preferred_model: Optional[str] = None) -> Dict[str, Any]:
        ...
        if HAS_OPENAI and os.getenv("OPENAI_API_KEY"):
            providers["openai"] = {
                ...
                "model": (
                    preferred_model
                    if preferred == "openai" and preferred_model
        ...
        if HAS_OPENAI and os.getenv("GROQ_API_KEY"):
            providers["groq"] = {
                ...
                "model": (
                    preferred_model
                    if preferred == "groq" and preferred_model
        ...
        if HAS_ANTHROPIC and os.getenv("ANTHROPIC_API_KEY"):
            providers["anthropic"] = {
                ...
                "model": (
                    preferred_model
                    if preferred == "anthropic" and preferred_model
    ...
    def __init__(self):
        ...
        if HAS_TRANSFORMERS and HAS_TORCH:
            try:
                ...
                # Latest-revision download is intentional for this CLI — pinning a
                # specific revision would prevent users from picking up upstream fixes.
                self.model = BertModel.from_pretrained(self.standard_repo)  # nosec B615
                self.tokenizer = BertTokenizer.from_pretrained(self.standard_repo)  # nosec B615
                if self.device.type == "mps":
                    self.model = self.model.half()
                self.model.to(self.device).eval()
                self.mode, self.available = "torch", True
        ...
        try:
            ...
            if self.mode == "ct2":
                ...
                def to_ids(tokens):
                    ...
                    return torch.tensor([self.tokenizer.cls_token_id] + ids + [self.tokenizer.sep_token_id]).to(
                        self.device
                    )
...
class FastAlignAligner:
    ...
    def __init__(self):
        ...
        try:
            import fast_align  # noqa: F401  (availability probe)
        ...
        if preferred and preferred != "auto":
            if preferred == "lindat":
                ...
                if HAS_FAST_ALIGN:
                    ...
                    logger.warning("fast_align requested but binary/package not found.")
...
class UltimateDocumentTranslator:
    ...
    def __init__(
        ...
        if mode in [
            TranslationMode.LLM_WITH_ALIGN,
            TranslationMode.LLM_WITHOUT_ALIGN,
    ...
    def extract_paragraph(self, para: Paragraph) -> TranslatableParagraph:
        ...
        def get_resolved_font_name(p):
            ...
            curr_style = p.style
            while curr_style:
                if curr_style.font.name:
                    return curr_style.font.name
            ...
            runs.append(
                FormatRun(
                    ...
                    font_size=(
                        run.font.size.pt
                        if run.font.size
                        else (para.style.font.size.pt if para.style.font.size else 12.0)
                    ),
                    font_color=f_color,
                )
            )
    ...
    def apply_aligned_formatting(
        ...
        try:
            ...
            if layout.get("left_indent") is not None:
                pf.left_indent = layout["left_indent"]
            if layout.get("right_indent") is not None:
                pf.right_indent = layout["right_indent"]
            if layout.get("first_line_indent") is not None:
                pf.first_line_indent = layout["first_line_indent"]
            if layout.get("line_spacing") is not None:
                pf.line_spacing = layout["line_spacing"]
            if layout.get("space_before") is not None:
                pf.space_before = layout["space_before"]
            if layout.get("space_after") is not None:
                pf.space_after = layout["space_after"]
        ...
        try:
            for run in para.runs:
                if run._element.xpath(".//w:drawing | .//w:pict"):
                    logger.debug("Skipping paragraph with drawing/image")
                    return False
        except Exception:
            pass
        ...
        try:
            ...
            if field_chars:
                ...
                if not is_footnote_field:
                    logger.debug("Skipping paragraph with field")
                    return False
        except Exception:
            pass
    ...
    def log_document_info(self, doc: Document, label: str):
        ...
        for i, section in enumerate(doc.sections):
            ...
            logger.debug(
                f"Section {i} | Margins: L:{section.left_margin.pt:.1f} R:{section.right_margin.pt:.1f} T:{section.top_margin.pt:.1f} B:{section.bottom_margin.pt:.1f}"
            )
            logger.debug(
                f"Section {i} | Gutter: {section.gutter.pt:.1f}pt | Header-Dist: {section.header_distance.pt:.1f}pt"
            )
    ...
    def log_para_trace(self, para: Paragraph, label: str):
        ...
        logger.debug(
            f"  > Indent-L: {pf.left_indent.pt if pf.left_indent else 0:.1f}pt | Spacing-A: {pf.space_after.pt if pf.space_after else 0:.1f}pt"
        )
    ...
    async def translate_document(self, input_path: Path, output_path: Path):
        ...
        translatable = [
            (para, ctx) for para, ctx in all_paras if para.text.strip() and self.is_paragraph_safe_to_translate(para)
        ]
...
Environment Variables:
  OPENAI_API_KEY, ANTHROPIC_API_KEY - Required for LLM backends.
    ...
    # --- LOGO & STATUS HEADER ---
    print(f"\n{'=' * 60}")
    print("🌍 DOCUMENT TRANSLATOR - PRODUCTION v12")
    print(f"{'=' * 60}")
