...
NOTES_HEADER_RE = re.compile(
    r"^\s*#{1,6}\s*(end\s?notes?|notes|footnotes|anmerkungen|endnoten|fußnoten|fussnoten|references)\s*$",
    re.IGNORECASE,
)
...
def split_body_notes(md_text: str) -> tuple[str, str]:
    ...
    for i, line in enumerate(lines):
        if NOTES_HEADER_RE.match(line):
            return "\n".join(lines[:i]), "\n".join(lines[i + 1 :])
    raise SystemExit("Could not find a notes section header (e.g. '## Endnotes').")
...
def generate_reference_docx(out_path: Path, body_font: str, body_size_pt: float, heading_font: str) -> None:
    ...
    try:
        default = subprocess.check_output(["pandoc", "--print-default-data-file=reference.docx"])
    except subprocess.CalledProcessError as e:
        raise SystemExit(f"pandoc cannot emit default reference docx: {e}") from e
...
def footnotes_to_endnotes(docx_path: Path) -> None:
    ...
    ct = ct.replace("wordprocessingml.footnotes+xml", "wordprocessingml.endnotes+xml")
...
def convert(
    ...
    try:
        ...
        if not keep_bold:
            ...
            if stripped:
                print(
                    f"info: stripped paragraph-wide bold from {stripped} paragraphs (use --keep-bold to disable)",
                    file=sys.stderr,
                )
        ...
        if missing:
            print(
                f"warning: body cites [N] without matching note: {sorted(missing)}",
                file=sys.stderr,
            )
        if unused:
            print(f"warning: note defs never cited: {sorted(unused)}", file=sys.stderr)

        md_rewritten.write_text(build_footnoted_markdown(new_body, notes), encoding="utf-8")
        ...
        if ref_doc_arg is not None:
            ...
            try:
                generate_reference_docx(ref_doc, body_font, body_size_pt, heading_font)
            except Exception as e:
                print(
                    f"warning: failed to build reference docx ({e}); falling back to pandoc default styles",
                    file=sys.stderr,
                )
                ref_doc = None
        ...
        if strip_rsids:
            ...
            if removed:
                print(
                    f"info: stripped {removed} rsid/paraId attributes (--no-strip-rsids to disable)",
                    file=sys.stderr,
                )

        print(f"wrote {output_path}  (refs in body: {len(used_nums)}, note defs: {len(notes)}, notes-as: {kind})")
        if keep:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--heading-font", default="Arial", help="heading font for the reference docx")
    p.add_argument(
        "--keep-bold",
        action="store_true",
    ...
    output_path = input_path.with_suffix(".docx") if args.output is None else args.output.expanduser().resolve()
    convert(
        input_path,
        output_path,
        args.notes,
